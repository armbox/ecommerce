...
class FulfillFrozenBaskets(EdxOrderPlacementMixin):
    ...
    def fulfill_basket(self, basket_id, site):
        ...
        if not order:
            ...
            self.payment_processor = _get_payment_processor(successful_payment_notification.processor_name)
            ...
            handled_response = HandledProcessorResponse(
                ...
                card_number=successful_payment_notification.response.get('req_card_number', None),
                card_type=successful_payment_notification.response.get('req_card_type', 'Paypal Account'),
            )
            ...
            try:
                ...
                logger.info('Successfully create order for basket %d'. basket.id)
                return True
